...
public class v1_20_R1 extends VersionUtils {
    ...
    void openMenu(SignMenuFactory.Menu menu, Player player) {
        MenuOpener_v1_20_R1.openMenu(menu, player);
    }

    @Override
    public void injectConnection(Player player) throws IllegalAccessException, NoSuchFieldException {
        Field field = ((CraftPlayer) player).getHandle().c.getClass().getDeclaredField("h");
        field.setAccessible(true);
        NetworkManager netManager = (NetworkManager) field.get(((CraftPlayer) player).getHandle().c);
        netManager.m.pipeline().addBefore("packet_handler", "ecs_listener", new ChannelHandler_v1_20_R1(player));
    }
