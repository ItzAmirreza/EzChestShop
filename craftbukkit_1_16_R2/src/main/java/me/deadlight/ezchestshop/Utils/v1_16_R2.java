...
public class v1_16_R2 extends VersionUtils {
    ...
    void destroyEntity(Player player, int entityID) {
        ((org.bukkit.craftbukkit.v1_16_R2.entity.CraftPlayer) player).getHandle().playerConnection.sendPacket(new PacketPlayOutEntityDestroy(entityID));
    }
    ...
    void openMenu(SignMenuFactory.Menu menu, Player player) {
        MenuOpener_v1_16_R2.openMenu(menu, player);
    }

    @Override
    public void injectConnection(Player player) {
        ((CraftPlayer) player).getHandle().playerConnection.networkManager.channel.pipeline().addBefore("packet_handler", "ecs_listener", new ChannelHandler_v1_16_R2(player));
    }
